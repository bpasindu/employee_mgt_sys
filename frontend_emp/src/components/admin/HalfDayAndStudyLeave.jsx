...
export default function HalfDayAndStudyLeave({ halfDayList = [], studyLeaveList = [] }) {
  const halfDays = halfDayList.length > 0 ? halfDayList : [
    { id: 18, name: 'Amal Perera', initials: 'AP', department: 'Finance', half_day_type: 'Morning Half Day', time_slot: '8:30 AM – 12:30 PM', reason: 'Personal appointment' },
    { id: 19, name: 'Tharushi Silva', initials: 'TS', department: 'HR', half_day_type: 'Afternoon Half Day', time_slot: '1:00 PM – 5:00 PM', reason: 'University work' }
  ];

  const studyLeave = studyLeaveList.length > 0 ? studyLeaveList[0] : {
    id: 20, name: 'Ravindu Perera', initials: 'RP', department: 'IT Department', position: 'Associate Developer', time_slot: '9:00 AM – 4:00 PM', reason: 'University examination'
  };
          ...
          <div className="space-y-4">
            {halfDays.map((emp) => (
              <div key={emp.id || Math.random()} className="border border-slate-100 bg-slate-50/50 rounded-2xl p-4">
                ...
                <p className="text-xs text-slate-500 mt-1 font-medium">{emp.reason}</p>
              </div>
            ))}
          </div>
        </div>
        ...
        <div>
          ...
          {studyLeave && (
            <div className="bg-[#eaf4fd] border border-blue-200/70 rounded-2xl p-6 relative overflow-hidden">
              ...
              <p className="text-xs text-slate-600 font-medium">{studyLeave.reason}</p>
            </div>
          )}
        </div>
      </div>
